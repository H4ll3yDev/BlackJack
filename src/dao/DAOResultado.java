...
public class DAOResultado {
    ...
    private static final String SQL_INSERTAR = "INSERT INTO resultado (partida_id, jugador_id, puntuacion, apuesta, es_ganador) " +
            								   "VALUES (?, ?, ?, ?, ?)";

    private static final String SQL_BUSCAR_POR_PARTIDA = "SELECT id, partida_id, jugador_id, puntuacion, apuesta, es_ganador " +
            											 "FROM resultado WHERE partida_id = ?";

    private static final String SQL_BUSCAR_POR_JUGADOR = "SELECT id, partida_id, jugador_id, puntuacion, apuesta, es_ganador " +
    													 "FROM resultado WHERE jugador_id = ? ORDER BY partida_id DESC";

    private static final String SQL_RANKING = "SELECT j.nombre_usuario, " +
    										  "       j.partidas_ganadas, " +
    										  "       j.saldo, " +
    ...
    public int insertar(int partidaId, int jugadorId, int puntuacion, int apuesta, boolean esGanador) {
    ...
    public void insertarTodos(int partidaId, List<Jugador> jugadores, int[] puntuaciones, int[] apuestas, int indiceGanador) {

        for (int i = 0; i < jugadores.size(); i++) {
        	
            boolean esGanador = (i == indiceGanador);
            insertar(partidaId, jugadores.get(i).getId(), puntuaciones[i], apuestas[i], esGanador);
    ...
    public List<String[]> buscarPorPartida(int partidaId) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String[] fila = {String.valueOf(rs.getInt("jugador_id")), String.valueOf(rs.getInt("puntuacion")),
    ...
    public List<String[]> obtenerRanking() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String[] fila = {rs.getString("nombre_usuario"), String.valueOf(rs.getInt("partidas_ganadas")),
